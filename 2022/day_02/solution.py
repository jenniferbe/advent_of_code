...
SCORES_DICT = {"A": ROCK, "B": PAPER, "C": SCISSORS,"X": ROCK, "Y": PAPER, "Z": SCISSORS}
WIN_LOSE_PAIRS = {(ROCK, SCISSORS), (SCISSORS, PAPER), (PAPER, ROCK)}

def play_round(p1, p2):
    if (p1 == p2):
        return (DRAW, DRAW)
    elif ((p1, p2) in WIN_LOSE_PAIRS):
        return (WIN, LOSE)
    else:
        return (LOSE, WIN)
...
def calculate_score(input_file):
    ...
    while (line):
        p1, p2 = line.split(" ")
        p1, p2 = SCORES_DICT[p1], SCORES_DICT[p2]
        round_scores = play_round(p1, p2)

        p1_score += p1 + round_scores[0]
        p2_score += p2 + round_scores[1]

        line = f.readline().strip()
    return p1_score, p2_score

print("Scores: {}".format(calculate_score(input_file)))
...
SCORES_DICT_2 = {"A": ROCK, "B": PAPER, "C": SCISSORS, "X": LOSE, "Y": DRAW, "Z": WIN}
WINNER_TO_LOSER = { winner: loser for winner, loser in WIN_LOSE_PAIRS }
LOSER_TO_WINNER = { loser: winner for winner, loser in WIN_LOSE_PAIRS }


def find_scores(p1, p2_result):
    if (p2_result == DRAW):
        return (DRAW, p1)
    elif (p2_result == LOSE):
        return (WIN, WINNER_TO_LOSER[p1])
    else:
        return (LOSE, LOSER_TO_WINNER[p1])

def backtrack_scores(input_file):
    f = open(input_file, "r")
    line = f.readline().strip()
    ...
    while (line):
        p1, p2 = line.split(" ")
        p1, p2 = SCORES_DICT_2[p1], SCORES_DICT_2[p2]
        round_scores = find_scores(p1, p2)
...
print("Scores: {}".format(backtrack_scores(input_file)))
