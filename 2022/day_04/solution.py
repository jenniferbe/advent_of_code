...
def format_assignment(line):
    assignments = line.split(",")
    r1 = map(lambda x : int(x), assignments[0].split("-"))
    r2 = map(lambda x : int(x), assignments[1].split("-"))
    return r1, r2

def is_in_range(r1, r2):
    return (r1[0] <= r2[0] <= r1[1]) and (r1[0] <= r2[1] <= r1[1]) 

def count_duplicate_work(input_file):
    f = open(input_file, "r")
    line = f.readline().strip()

    duplicate_count = 0
    while (line):
        r1, r2 = format_assignment(line)

        if (is_in_range(r1, r2) or is_in_range(r2, r1)):
            duplicate_count += 1
...
print("Duplicate work: {}".format(count_duplicate_work(input_file)))
...
def count_overlaps(input_file):
    ...
    while (line):
        ...
        if (is_overlap(r1, r2) or is_overlap(r2, r1)):
            overlap_count += 1
...
print("Overlapping work: {}".format(count_overlaps(input_file)))
