...
def split_compartments(string):
    ...
    return set(string[:compartment_length]), set(string[compartment_length:])

def get_compartment_type(s1, s2):
    return list(s1 & s2)[0]

# a-z (ord 97-122), A-Z (ord 65-90)
def get_letter_priority(letter):
    letter_ord = ord(letter)

    if (letter_ord > 90):
        return letter_ord - 96
    return letter_ord - 38

def get_priority(line):
    c1, c2 = split_compartments(line)
    compartment_type = get_compartment_type(c1, c2)
    priority = get_letter_priority(compartment_type)
    return priority

def find_sum_priorities(input_file):
    f = open(input_file, "r")
    line = f.readline().strip()

    sum_priorities = 0
    while (line):
        sum_priorities += get_priority(line)
        line = f.readline().strip()
    f.close()
    return sum_priorities

print("Sum priorities: {}".format(find_sum_priorities(input_file)))
...
def get_group_priority(elf1, elf2, elf3):
    badge = get_group_badge(elf1, elf2, elf3)
    return get_letter_priority(badge)

def find_group_priorities(input_file):
    f = open(input_file, "r")
    elf1, elf2, elf3 = f.readline().strip(), f.readline().strip(), f.readline().strip()

    sum_priorities = 0
    while (elf3):
        sum_priorities += get_group_priority(elf1, elf2, elf3)
        elf1, elf2, elf3 = f.readline().strip(), f.readline().strip(), f.readline().strip()
    f.close()
    return sum_priorities

print("Group priorities: {}".format(find_group_priorities(input_file)))
