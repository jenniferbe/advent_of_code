...
def process_moves(file, stacks):
    ...
    while (line): 
        moves = line.split(" ")
        count, origin, dest = int(moves[1]), int(moves[3])-1, int(moves[5])-1

        for _ in range(count):
            crate = stacks[origin].pop(0)
...
def get_top_stacks(input_file):
    f = open(input_file, "r")
    stacks = process_stacks(f)
    moved_stacks = process_moves(f, stacks)
    f.close()
    return reduce(lambda x, y: x + y[0], moved_stacks, "")

print("Top stacks: {}".format(get_top_stacks(input_file)))
...
def get_top_stacks_9001(input_file):
    f = open(input_file, "r")
    stacks = process_stacks(f)
    moved_stacks = process_moves(f, stacks)
    f.close()
...
print("Top stacks: {}".format(get_top_stacks_9001(input_file)))
