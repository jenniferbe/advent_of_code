...
update the position of the tail if the step means the head is no longer adjacent to
the tail. Visually, these motions occur as follows (s marks the starting position as
a reference point):
...
least once. In this diagram, s again marks the starting position (which the tail
...
def make_move(head, tail):
    ...
    if (abs(diff_x) > 1) and (diff_y == 0):
        diff = 1 if diff_x > 0 else -1
        tail = tail[0] + diff, tail[1]
    # move towards y
    elif (abs(diff_y) > 1) and (diff_x == 0):
        diff = 1 if diff_y > 0 else -1
        tail = tail[0], tail[1] + diff
    # move diagonal
    elif (abs(diff_x) > 1) or (abs(diff_y) > 1):
        diff_1 = 1 if diff_x > 0 else -1
        diff_2 = 1 if diff_y > 0 else -1
        tail = tail[0] + diff_1, tail[1] + diff_2
    return tail

def make_moves(action, head, tail):
    head = make_action(action, head)
    tail = make_move(head, tail)
    return head, tail

def get_unique_grid_positions(input_file):
    ...
    while (line):
        ...
        for _ in range(int(steps)):
            head, tail = make_moves(action, head, tail)
            tail_positions.add(tail)

        line = f.readline().strip()
    return len(tail_positions)

print("# of unique grid positions: {}".format(get_unique_grid_positions(input_file)))

"""
A rope snaps! Suddenly, the river is getting a lot closer than you remember. The
bridge is still there, but some of the ropes that broke are now whipping toward you
as you fall through the air!

The ropes are moving too quickly to grab; you only have a few seconds to choose how
to arch your body to avoid being hit. Fortunately, your simulation can be extended
...
Each knot further down the rope follows the knot in front of it using the same rules
as before.

Using the same series of motions as the above example, but with the knots marked H,
1, 2, ..., 9, the motions now occur as follows:
...
Simulate your complete series of motions on a larger rope with ten knots. How many
positions does the tail of the rope visit at least once?

Solution: 2445
"""

def make_multiple_moves(action, rope):
    rope[0] = make_action(action, rope[0])
    for i in range(1, len(rope)):
        rope[i] = make_move(rope[i-1], rope[i])
    return rope

def get_long_unique_grid_positions(input_file, length):
    ...
    while (line):
        action, steps = line.split(" ")
        for _ in range(int(steps)):
            rope = make_multiple_moves(action, rope)
            tail_positions.add(rope[length-1])

        line = f.readline().strip()
    return len(tail_positions)

print("# of unique grid positions: {}".format(get_long_unique_grid_positions(input_file, 10)))
